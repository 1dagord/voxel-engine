...
def get_ao(local_pos, world_pos, world_voxels, plane):
	"""
		Gets ambient occlusion value for voxel based on
		adjacent voxels
		(adds depth in corners)
	"""
	x, y, z = local_pos
	wx, wy, wz = world_pos
...
def pack_data(x, y, z, voxel_id, face_id, ao_id, flip_id):
	"""
		Reduces data needed for all attributes from 56 bits to 32
	...
	for x in range(CHUNK_SIZE):
		for y in range(CHUNK_SIZE):
			for z in range(CHUNK_SIZE):
				...
				# iterate only over voxels that are not empty space
				if not voxel_id:
					continue
				...
				if is_void((x, y + 1, z), (wx, wy + 1, wz), world_voxels):
					...
					# creates two triangles from vertices to create top face
					if flip_id:
